...
public class FilesActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        lv_file = (RecyclerView) findViewById(R.id.lv_file);
...
//        }
        ...
        fileList = yunSqliteHelper.getFileList(MainApplication.getLoginUserInfo().userId, fileType);

        if (fileList.size() > 0) {
            adapter = new FileAdapter(fileList, FilesActivity.this);

            lv_file.setLayoutManager(new LinearLayoutManager(this));

            lv_file.setAdapter(adapter);

        } else {
            Toast.makeText(FilesActivity.this, "你暂时还没有上传该类文档，快去上传吧", Toast.LENGTH_SHORT).show();
        }
    }
